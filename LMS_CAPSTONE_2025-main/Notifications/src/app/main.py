...
from routers.notifRouter import router as notif_router
...
app.mount("/notif/static", StaticFiles(directory=static_dir), name="static")
...
app.include_router(notif_router, prefix="/notif")
...
async def root(request: Request):
    ...
    if login_token:
        try:
            ...
            return RedirectResponse(url="/notif/main", status_code=status.HTTP_303_SEE_OTHER)
        except HTTPException:
            pass
    return RedirectResponse(url="/notif/manager-login", status_code=status.HTTP_303_SEE_OTHER)

if __name__ == "__main__":
    uvicorn.run(
        app="main:app",
        host="0.0.0.0",
        port=8008,  # Use a different port
