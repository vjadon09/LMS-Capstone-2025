...
async def root(request: Request):
    ...
    if login_token:
        try:
            ...
            return RedirectResponse(url="/search/home", status_code=status.HTTP_303_SEE_OTHER)
        except HTTPException:
            pass
    return RedirectResponse(url="/search/login", status_code=status.HTTP_303_SEE_OTHER)
